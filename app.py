...
def calc_heating_rate_and_predict(readings, city_tz):
    ...
    if len(today_readings) < 2:
        return None, None, ["Need more readings from today"]
    
    today_readings.sort(key=lambda x: x['hour'])
    
    factors = ["**Today's NWS Readings:**"]
    ...
    factors.append(f"**Latest:** {latest['temp']:.1f}°F @ {latest['time_str']}")
    
    current_hour = now.hour + now.minute/60
    
    if current_hour >= 9 and len(today_readings) >= 2:
        morning_start = next((r for r in today_readings if r['hour'] >= 6), today_readings[0])
        ...
        factors.append(f"**Hours to peak:** {hours_to_peak:.1f}")
        factors.append(f"**Remaining rise:** {remaining_rise:.1f}°F")
        factors.append(f"**Prediction: {projected_low:.0f}-{projected_high:.0f}°F**")
